...
@Component({
  selector: 'app-carddeck-edit-admin-routed',
  templateUrl: './carddeck-edit-admin-routed.component.html',
  styleUrls: ['./carddeck-edit-admin-routed.component.css']
})
export class CarddeckEditAdminRoutedComponent implements OnInit {
  id: number = 0;
    ...
    this.oCarddeckService.getOne(this.id).subscribe({
      next: (data: ICarddeck) => {
        ...
        this.oForm = <FormGroup>this.oFormBuilder.group({
          ...
          copies: [data.copies, [Validators.required, Validators.minLength(1), Validators.maxLength(1)]],
          iddeck: [data.deck.id, [Validators.required, Validators.pattern(/^\d{1,6}$/)]],
          idcard: [data.card.id, [Validators.required, Validators.pattern(/^\d{1,6}$/)]]
        });
    ...
    if (this.oForm.valid) {
      this.oCarddeckService.updateOne(this.oCarddeck2Send).subscribe({
        next: (data: number) => {
          ...
          this.modalContent = "Carddeck " + data + " edited";
          this.showModal(data);
        }
      })
    }
